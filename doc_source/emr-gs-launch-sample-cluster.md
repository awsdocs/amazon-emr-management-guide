...
| Release | emr\-5\.26\.0 | This option specifies the Amazon EMR release version to use when the cluster is created\. The Amazon EMR release determines the version of open\-source applications, such as Hadoop and Hive, that Amazon EMR installs\. The label for the latest release version is selected by default\. You can select an earlier Amazon EMR release if you need different versions of open\-source applications for compatibility with your solution\. Some Amazon EMR features and applications may not be available when using earlier Amazon EMR release versions, so recommend that you use the latest release version whenever possible\. For more information about each Amazon EMR release version, see [Amazon EMR Release Guide](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/)\. | 
